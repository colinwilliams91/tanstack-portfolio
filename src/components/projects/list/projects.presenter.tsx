    ...
    <div>
      ...
      <div className="grid gap-4.5 md:grid-cols-2 lg:grid-cols-3">
        {data?.slice().sort((a, b) => b.stargazers_count - a.stargazers_count).map((project) => (
          <a href={project.url} key={project.id} className="hover-3d" aria-label={`${LABELS.PROJECT.VIEW_PROJECT}: ${project.name}`}>
            <div className="card bg-base-200 shadow-sm hover:shadow-md rounded-box">
              <div className="card-body text-sm">
                <h2 className="card-title">{project.name}</h2>
                <img className="mask mask-circle w-16 h-16" src={project.owner.avatar_url} alt={`${LABELS.IMAGES.PROJECT_OWNER}: ${project.owner.login}`} />
